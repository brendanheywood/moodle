 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class defaultuserrole extends security_check {
    ...
    public function __construct() {
        ...
        if (!$defaultrole = $DB->get_record('role', ['id' => $CFG->defaultuserroleid])) {
            ...
            $this->summary = get_string('check_defaultuserrole_notset', 'report_security');
            return;
        }
        ...
        if ($riskycount or !$legacyok) {
            ...
            $this->summary = get_string('check_defaultuserrole_error', 'report_security', role_get_name($defaultrole));

        } else {
            $this->status = check::OK;
            $this->summary = get_string('check_defaultuserrole_ok', 'report_security');
        }

        $this->details = get_string('check_defaultuserrole_details', 'report_security');
