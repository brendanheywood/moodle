...
class publicpaths extends check {
    ...
    public function get_pathsets() {
        ...
        return [
            ...
            [
                ...
                // We add this as a default security policy, but also as a check
                // that when the dot files are hidden we don't also block .well-known
                // See https://tools.ietf.org/html/rfc5785 for more info.
                '200'       => [
                    '.well-known/security.txt',
                ],
                'summary'   => get_string('check_dotfiles_info', 'report_security'),
            ],
            [
    ...
    //
    public function get_combined_pattern() {
    }
    ...
    public function get_result(): result {
        ...
        foreach ($paths as $path) {
            foreach (['200', '404'] as $expected) {
                if (!isset($path[$expected])) continue;
